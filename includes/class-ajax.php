...
class Ajax {
	...
	public function __construct( Webhook $webhook, Logger $logger, Payload $payload ) {
		...
		add_action( 'wp_ajax_dwm_save_webhook', [ $this, 'handle_save_webhook' ] );
		add_action( 'wp_ajax_dwm_delete_webhook', [ $this, 'handle_delete_webhook' ] );
		add_action( 'wp_ajax_dwm_toggle_webhook', [ $this, 'handle_toggle_webhook' ] );
		add_action( 'wp_ajax_dwm_test_webhook', [ $this, 'handle_test_webhook' ] );
		add_action( 'wp_ajax_dwm_retry_delivery', [ $this, 'handle_retry_delivery' ] );
		add_action( 'wp_ajax_dwm_clear_logs', [ $this, 'handle_clear_logs' ] );
	}
	...
	public function handle_save_webhook(): void {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied.', 'dragon-webhook-manager' ) ] );
		}
		...
		$data = [
			...
			'headers'          => $this->parse_headers( $_POST['headers'] ?? '' ),
		...
		if ( empty( $data['name'] ) || empty( $data['trigger_event'] ) || empty( $data['url'] ) ) {
			wp_send_json_error( [ 'message' => __( 'Name, trigger, and URL are required.', 'dragon-webhook-manager' ) ] );
		}

		// Validate URL
		if ( ! filter_var( $data['url'], FILTER_VALIDATE_URL ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid URL format.', 'dragon-webhook-manager' ) ] );
		}
	...
	public function handle_delete_webhook(): void {
		...
		if ( ! $id ) {
			wp_send_json_error( [ 'message' => __( 'Invalid webhook ID.', 'dragon-webhook-manager' ) ] );
		}
		...
		if ( ! $result ) {
			wp_send_json_error( [ 'message' => __( 'Failed to delete webhook.', 'dragon-webhook-manager' ) ] );
		}

		wp_send_json_success( [ 'message' => __( 'Webhook deleted.', 'dragon-webhook-manager' ) ] );
	}
	...
	public function handle_toggle_webhook(): void {
		...
		if ( ! $result ) {
			wp_send_json_error( [ 'message' => __( 'Failed to toggle webhook.', 'dragon-webhook-manager' ) ] );
		}
	...
	public function handle_test_webhook(): void {
		...
		if ( ! $webhook ) {
			...
			$webhook = [
				...
				'url'              => esc_url_raw( $_POST['url'] ?? '' ),
				'method'           => sanitize_key( $_POST['method'] ?? 'POST' ),
				'headers'          => wp_json_encode( $this->parse_headers( $_POST['headers'] ?? '' ) ),
				'payload_template' => wp_unslash( $_POST['payload_template'] ?? '{}' ),
			];
		}

		if ( empty( $webhook['url'] ) ) {
			wp_send_json_error( [ 'message' => __( 'URL is required.', 'dragon-webhook-manager' ) ] );
		}
		...
		if ( $result['success'] ) {
			...
			wp_send_json_error( [
				'message'       => $result['error_message'] ?: __( 'Webhook delivery failed.', 'dragon-webhook-manager' ),
				'response_code' => $result['response_code'],
				'response_body' => $result['response_body'],
				'duration_ms'   => $result['duration_ms'],
			] );
	...
	public function handle_retry_delivery(): void {
		...
		if ( ! $log ) {
			wp_send_json_error( [ 'message' => __( 'Log not found.', 'dragon-webhook-manager' ) ] );
		}

		$webhook = $this->webhook->get( $log['webhook_id'] );

		if ( ! $webhook ) {
			wp_send_json_error( [ 'message' => __( 'Webhook not found.', 'dragon-webhook-manager' ) ] );
		}
		...
		if ( $result['success'] ) {
			...
			wp_send_json_error( [
				'message'       => $result['error_message'] ?: __( 'Retry failed.', 'dragon-webhook-manager' ),
				'response_code' => $result['response_code'],
			] );
		}
	}
	...
	public function handle_clear_logs(): void {
		...
		wp_send_json_success( [ 'message' => __( 'Logs cleared.', 'dragon-webhook-manager' ) ] );
	}
	...
	private function get_sample_context( string $trigger_event ): array {
		...
		if ( in_array( $trigger_event, [ 'post_published', 'post_updated', 'post_trashed' ], true ) ) {
			$context['post'] = (object) $sample_post;
		}

		if ( in_array( $trigger_event, [ 'user_registered', 'user_login' ], true ) ) {
			$context['user'] = (object) $sample_user;
		}

		if ( in_array( $trigger_event, [ 'comment_submitted', 'comment_approved' ], true ) ) {
